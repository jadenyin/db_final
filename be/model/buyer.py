...
class buyer_functions(session.ORMsession):
    ...
    def retrieve(self, key_word: str, term: str, store_id: str) -> (int, str, list):
        ...
        if store_id != '' and not self.store_id_exist(store_id):
            return error.error_non_exist_store_id(store_id) + ([],)
        if term == 'title':
            book_id = self.db_session.query(ct.Book.book_id).filter(ct.Book.title.like('%' + key_word + '%')).all()
        # 标签搜索必须精准
        elif term == 'tag':
            book_id = self.db_session.query(ct.Book.book_id) \
                .filter(or_(ct.Book.tag1 == key_word, ct.Book.tag2 == key_word, ct.Book.tag3 == key_word)).all()
        elif term == 'content':
            # sql="select book_id from book where to_tsvector(tsv_content) @@ plainto_tsquery(%s)"
            # data=(key_word,)
            # book_id=self.db_session.execute(sql,data)
